...
func img_read(pathname string) error {
...
var device_param = flag.String("device", "test003_outgoing", "Device name")
						...
						case StatusAuthorized:
							{
								...
								for _, f := range files {
									if strings.HasSuffix(strings.ToUpper(f.Name()), "."+IMAGES_EXT) {
										appStream.frames = append(appStream.frames, IMAGES_FOLDER+"/"+f.Name())
									}
								}
						...
						case StatusIOStarted:
							{
								...
								if timeout >= MAX_DELTA {
									...
									go func() {
										check(img_read(appStream.CurFrame()))
										appStream.NextFrame()
